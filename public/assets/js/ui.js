...
const krds_mainMenuPC = {
  ...
  toggleMainMenu(mainTrigger) {
    ...
    if (!isActive && mainTrigger.nextElementSibling) {
      ...
      if (!isDropDown) {
        const scrollbarWidth = common.getScrollbarWidth();
        this.toggleBackdrop(true);
        this.toggleScrollbar(true, scrollbarWidth);
  ...
  toggleScrollbar(isEnabled, scrollbarWidth = 0) {
    const isScrollNeeded = document.body.scrollHeight > window.innerHeight;
    if (isEnabled && isScrollNeeded) {
      document.body.classList.add("hasScrollY");
